...
def ResNet(amt_64, amt_128, amt_256, amt_512, augmentation = False):
    ...
    if augmentation:
        model.add(keras.layers.GaussianNoise(stddev=4)) # Random noise
...
def run_and_save(path: Path, Data: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], augmentation: bool, learning_rate: float, resnet_size: Tuple[int, int, int, int], do_early_stopping=True):
